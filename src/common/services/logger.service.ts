...
export class LoggerService extends WinstonLogger {
  ...
  http(status, logFormat): void {
    // 根据状态码，进行日志类型区分
    if (status >= 500) {
      this.error(`${Chalk.hex('#e50048')(status)} ` + logFormat);
    } else if (status >= 400) {
      this.warn(`${Chalk.hex('#e4e700')(status)} ` + logFormat);
    } else {
      this.info(`${Chalk.hex('#00c358')(status)} ` + logFormat);
    }
  }
  ...
  exception(status, data, req, error: any): void {
    ...
    const logFormat = `${req.method} 请求地址: ${req.originalUrl} 请求IP: ${
      req.ip
    }\n 响应内容: ${JSON.stringify(data)}\n 错误信息: ${Chalk.hex('#e50048')(
      error.stack.split('at ')[1],
    )}`;

    this.error(`${Chalk.hex('#e50048')(status)} ` + logFormat, error);
  }
